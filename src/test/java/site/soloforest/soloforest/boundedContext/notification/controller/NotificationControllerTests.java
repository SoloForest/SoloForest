...
public class NotificationControllerTests {
	...
	void t002() throws Exception {
		// 리다이렉션된 URL을 얻기 위한 첫 번째 요청
		String redirectedUrl = mvc.perform(post("/notification")
				.param("notificationId", "13")
				.with(csrf()))
			.andExpect(status().is3xxRedirection())
			.andReturn().getResponse().getRedirectedUrl();

		// 리다이렉션된 URL로 새로운 요청을 만들어 HTML 내용을 검사
		mvc.perform(get(redirectedUrl))
			.andExpect(status().isOk())
			.andExpect(content().string(containsString("건조증")))
			.andExpect(content().string(containsString("넹의 답글")));
	}
